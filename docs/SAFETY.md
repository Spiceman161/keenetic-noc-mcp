...
tools. Writable mode exposes only `set_interface_state`, `restart_interface`,
and `save_config` in v0.1, plus raw POST only when explicitly enabled.

Mutations default to `dry_run=true`. A real call needs both `dry_run=false` and
...
Mutation attempts are appended to `audit.jsonl` below `KEENETIC_STATE_DIR`.
